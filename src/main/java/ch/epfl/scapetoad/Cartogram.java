...
public class Cartogram extends com.sun.swing.SwingWorker {
    ...
    /**
     * If the advanced options are enabled, this is the number of iterations the
     * diffusion algorithm is run on the cartogram grid.
     */
    // int mDiffusionIterations = 3;

    /**
     * The maximum running time in seconds. After this amount of time, the
     * cartogram computation is finalized. This is to avoid that the computation
     * lasts for a very very long time. The default value is 3 hours.
     */
    ...
    public Object construct() {
        ...
        try {
            ...
            mGrid.computeOriginalDensityValuesWithLayer(masterLayer,
                    mMasterAttribute, mMasterAttributeIsDensityValue);
            ...
            mCartogramWizard.updateRunningStatus(350,
            ...
            if (exceptionType == "java.lang.InterruptedException") {
                 ...
                 * PrintWriter(sw, true); e.printStackTrace(pw); pw.flush();
                 * sw.flush();
                 */
               ...
               * else { // Retrieve the complete stack trace and display.
               * StringWriter sw = new StringWriter(); PrintWriter pw = new
               * PrintWriter(sw, true); e.printStackTrace(pw); pw.flush();
               * sw.flush();
               * 
    ...
    private void updateEnvelope() {
        ...
        Envelope masterEnvelope = lyr.getFeatureCollectionWrapper()
                .getEnvelope();

        mEnvelope = new Envelope(masterEnvelope.getMinX(),
                masterEnvelope.getMaxX(), masterEnvelope.getMinY(),
    ...
    private Layer[] projectLayers() throws Exception {
        ...
        mMaximumSegmentLength = estimateMaximumSegmentLength();

        // Project the master layer.

        mCartogramWizard.updateRunningStatus(750, "Projecting the layers...",
                "Layer 1 of " + nlyrs);

        Layer masterLayer = mLayerManager.getLayer(mMasterLayer);
        CartogramLayer.regularizeLayer(masterLayer, mMaximumSegmentLength);
        mProjectedMasterLayer = CartogramLayer.projectLayerWithGrid(
                masterLayer, mGrid);
        ...
        for (int lyrcnt = 0; lyrcnt < nlyrs - 1; lyrcnt++) {
            mCartogramWizard.updateRunningStatus(800 + (lyrcnt + 1)
    ...
    /*
     * public int getDiffusionGridSize () { return mDiffusionGridSize; }
     * 
     * 
     * public void setDiffusionGridSize (int size) { mDiffusionGridSize = size;
     * }
     */

    /*
     * public int getDiffusionIterations () { return mDiffusionIterations; }
     * 
     * 
    ...
    private void createGridLayer() {
        ...
        // Create the layer.
        mDeformationGrid = new Layer("Deformation grid", Color.GRAY, fd,
                mLayerManager);
    ...
    private void createLegendLayer() {
        ...
        mLegendLayer = new Layer("Legend", Color.GREEN, fd, mLayerManager);
    ...
    public void produceComputationReport(Layer projectedMasterLayer) {
        ...
        if (mMasterAttributeIsDensityValue) {
        ...
        double meanError = CartogramLayer.computeCartogramSizeError(
                projectedMasterLayer, mMasterAttribute, masterLayer,
                "SizeError");
